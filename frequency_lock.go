...
func (this *LockTable) IsLock(key string, lock_time time.Duration) bool {
	...
	if item, ok := this.Items[key]; ok {
		...
		log.Infof("[LockTable] %s is limited\n", key)
		return true
	}
	this.Items[key] = item
	cleannerDuraction := this.CleanerDuraction
	this.Unlock()
	log.Infof("[LockTable] add %s to table\n", key)
...
func (this *LockTable) cleanerCheck() {
	this.Lock()
	defer this.Unlock()
	log.Infof("[LockTable] start timer cleaner Duraction after %.2f s\n", this.CleanerDuraction.Seconds())
	if this.Cleaner != nil {
		this.Cleaner.Stop()
	}
	...
	for key, item := range this.Items {
		...
		if now.Sub(createOn) >= lifeSpan {
			log.Infof("[LockTable] delete key %s", key)
